...
public class dreamloLeaderBoard : MonoBehaviour {
	...
	public string privateCode = "FukWGJ6CiEC1Aj1IpRg_vwX6ZqQNeUqUuoRDpLlbuSSA";
	public string publicCode = "5d01016e3ebb3b1870d0b44c";
	...
	IEnumerator AddScoreWithPipe(string playerName, int totalScore)
	{
		...
		UnityWebRequest www = new UnityWebRequest(dreamloWebserviceURL + privateCode + "/add-pipe/" + UnityWebRequest.EscapeURL(playerName) + "/" + totalScore.ToString());
		yield return www;
		//highScores = www.text;
	}
	...
	IEnumerator AddScoreWithPipe(string playerName, int totalScore, int totalSeconds)
	{
        ...
        UnityWebRequest www = new UnityWebRequest(dreamloWebserviceURL + privateCode + "/add-pipe/" + UnityWebRequest.EscapeURL(playerName) + "/" + totalScore.ToString()+ "/" + totalSeconds.ToString());
		yield return www;
		//highScores = www.text;
	}
	...
	IEnumerator AddScoreWithPipe(string playerName, int totalScore, int totalSeconds, string shortText)
	{
        ...
        UnityWebRequest www = new UnityWebRequest(dreamloWebserviceURL + privateCode + "/add-pipe/" + UnityWebRequest.EscapeURL(playerName) + "/" + totalScore.ToString() + "/" + totalSeconds.ToString()+ "/" + shortText);
		yield return www;
	//	highScores = www.text;
	}
	...
	{
		highScores = "";
        UnityWebRequest www = new UnityWebRequest(dreamloWebserviceURL +  publicCode  + "/pipe");
		yield return www;
		//highScores = www.text;
	}
	...
	{
		highScores = "";
        UnityWebRequest www = new UnityWebRequest(dreamloWebserviceURL +  publicCode  + "/pipe-get/" + UnityWebRequest.EscapeURL(playerName));
		yield return www;
	//	highScores = www.text;
	}
